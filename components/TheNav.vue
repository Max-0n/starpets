...
nav.nav(:class="{ 'nav--open': isOpen }")
  ...
  .nav__content
    .nav__menu
      NuxtLink(to="/" @click="close").nav__menu-item(:class="{ 'active': route.path === '/' }")
        UIBadge(color="orange" size="md")
          UIIcon(name="cart").size-24
        p.text-size-16.text-weight-500 Магазин
      
      NuxtLink(to="/products" @click="close").nav__menu-item(:class="{ 'active': route.path === '/products' }")
        UIBadge(color="blue" size="md")
          UIIcon(name="tag").size-24
        p.text-size-16.text-weight-500 Предметы
      
      NuxtLink(to="/sale" @click="close").nav__menu-item(:class="{ 'active': route.path === '/sale' }")
        UIBadge(color="purple" size="md")
          UIIcon(name="quest").size-24
        p.text-size-16.text-weight-500 Продажа
      
      NuxtLink(to="/currency" @click="close").nav__menu-item(:class="{ 'active': route.path === '/currency' }")
        UIBadge(color="pink" size="md")
          UIIcon(name="coin").size-24
        p.text-size-16.text-weight-500 Робуксы
      
      NuxtLink(to="/help" @click="close").nav__menu-item(:class="{ 'active': route.path === '/help' }")
